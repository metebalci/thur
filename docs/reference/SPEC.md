...
| 0x13 | VERIFY(6) | Medium-only verify (no host-data compare): re-reads N blocks, re-checks stored integrity, advances position. Faults map through `error_to_sense` — corrupt index record / chunk payload → MEDIUM ERROR + 0x11/0x00 (issue #105). |
| 0x19 | ERASE | |
