...
| 0x10 | WRITE FILEMARKS(6) | 🟩 Yes | M | 24-bit filemark count. Counts above 65 536 per command are refused with INVALID FIELD IN CDB (0x05/0x24) — a deliberate divergence from physical LTO (which completes any count): the count is untrusted and each filemark appends a zero-byte index record under the drive mutex, so an uncapped count would wedge the drive and grow the index until the disk fills (issue #132). Legitimate use is 0-2. The whole batch pays one index fsync, not one per mark. |
| 0x11 | SPACE(6) | 🟩 Yes | M | Records / Filemarks / EOD; sign-extended 24-bit count. Spacing over *records* (code 000b) **halts on the first filemark in the path of motion** (SSC-4 §7.5): CHECK CONDITION + NoSense + ASC/ASCQ 0x00/0x01 (Filemark detected) + FM=1, INFO = residual `count − moved`, position left on the EOP side of the mark going forward / the BOP side going reverse. The Linux st driver depends on that stop to keep its (file, block) model in sync — walking past the mark made a following WRITE land where the host didn't intend (issue #102). A short traversal without a filemark (`moved != count`) also terminates CHECK CONDITION with the residual in INFO; that sense is **direction-aware** (SSC-4 §7.5): a *forward* shortfall ran past recorded data → BlankCheck + ASC/ASCQ 0x00/0x05 (End-of-data) — the slow-MTEOM / Bareos path (issue #33); a *backward* shortfall can only have reached Beginning-of-Partition → NoSense + ASC/ASCQ 0x00/0x04 (Beginning-of-Partition/Medium) — never EOD (issue #73). Reporting BlankCheck on backward motion broke the Linux st driver's `MTBSF` reposition. A block-index read failure during the walk terminates with CHECK CONDITION — the same sense the data-read path maps that fault to: MEDIUM ERROR + 0x11/0x00 (UNRECOVERED READ ERROR) for a record that fails to decode (corrupt index sidecar, issue #105), HARDWARE ERROR + 0x44/0x00 for a pread I/O failure — instead of silently classifying the unreadable block as a record (issue #104). |
| 0x13 | VERIFY(6) | 🟩 Yes | O | Medium-only verify (BYTCMP treated as 0; no host-data compare): re-reads each of the N blocks from the head position and re-checks its stored integrity (chunk BLAKE3 / GCM tag / codec CRC), advancing the position. Faults surface with the truthful sense via `error_to_sense` — corrupt index record or chunk payload (hash- or codec-detected) → MEDIUM ERROR + 0x11/0x00 (issues #105, #108). |
...
| 0x80 | WRITE FILEMARKS(16) | 🟩 Yes | O | 32-bit count at CDB[12..16]. Same 65 536/command cap as the 6-byte form (issue #132). |
| 0x82 | ALLOW OVERWRITE | 🟩 Yes | O | Volatile flag, cleared on UNLOAD. While the barrier is set and the head is inside the existing partition span, a WRITE rewrites the block-index record at the head position in place (LTFS index rewrite) instead of truncating-and-appending — so a read-back at the head sees the new block and READ POSITION stays consistent (issue #116). |
| 0x8F | VERIFY(16) | 🟩 Yes | O | Same semantics as VERIFY(6) with a 64-bit count (cdb[4..12]). |
| 0x91 | SPACE(16) | 🟩 Yes | O | 64-bit count. Same semantics as SPACE(6): records-mode traversal halts on a filemark with NoSense 0x00/0x01 + FM=1 + residual (issue #102), and the residual + direction-aware EOD-vs-BOP termination is identical: forward shortfall → BlankCheck 0x00/0x05, backward shortfall → NoSense 0x00/0x04 (issue #73). One divergence from the 6-byte form: a residual outside the range of fixed-format sense's 4-byte INFORMATION field — reachable only through the 64-bit count — is reported with VALID=0 and no INFORMATION, per SPC-4 §4.5.2, rather than truncated (issue #104). |
...
| 0xA5 | MOVE MEDIUM | 🟩 Yes | M | Storage ↔ Drive ↔ I/E ↔ Storage. Emits MEDIUM CHANGED UA. Partition-fenced when the session is bound to a logical partition (CHAP user → partition mapping); cross-partition src/dst returns ILLEGAL REQUEST + 0x21/0x01. Reservation-gated (write-side — see 0x5F). A load that would mount a cartridge a `cartridge migrate` / tiering move currently holds is **refused** with NOT READY + OPERATION IN PROGRESS (0x04/0x07, transient — the host retries once the migration completes), and the changer rolls the inventory move back so library state stays consistent: the migrate's copy-then-flip-then-delete sequence assumes a stable chunk set, so appending mid-migration would orphan or drop chunks (issue #212). |
| 0xA6 | EXCHANGE MEDIUM | 🟩 Storage/IE only | O | Single **atomic** inventory transaction (one persist): the medium at src moves to dst1 and the medium at dst1 moves to dst2. The canonical two-element swap `dst2 == src` (what `mtx exchange A B` issues) is supported, and a refusal leaves the inventory untouched — no half-applied state (issue #191). Same partition fence applies to all three element addresses. Reservation-gated (write-side — see 0x5F). **Drive-involving exchanges are refused** with INVALID FIELD IN CDB (0x05/0x24): this path does not mirror drive loads/unloads into the drive manager, so allowing one would bind the data path to the wrong cartridge — silent data corruption (issue #133). Hosts use MOVE MEDIUM (load/unload) for drives; EXCHANGE swaps storage / I/E cartridges. |
...
| 0xB8 | READ ELEMENT STATUS | 🟩 Yes | M | All element types; VOLTAG / DVCID / Mixed / CurData / Access flags honored. The **starting element address is honored as a floor** and the **number-of-elements field caps** the descriptors returned: an address inside a type maps to its offset, an address below a type starts at id 0, and an address at/beyond a type's range yields an empty per-type page (typed request) or skips the type (type 0) — so paged inventory walks terminate instead of re-listing from element 0. When VOLTAG is requested the per-type page header sets **PVOLTAG** (byte 1 bit 7) so conforming parsers read the barcodes. **Not** partition-fenced — every session sees every element. `mtx` parses a zero-descriptor per-type page as `Transport Element Descriptor Length too short`, so dropping out-of-partition elements there breaks `mtx load`. Topology leak is contained: MOVE MEDIUM / EXCHANGE MEDIUM still refuse out-of-partition src/dst, and drive-LUN INQUIRY against an out-of-partition LUN still returns PQ=NoDevice, so the data path stays isolated. Reservation-gated (read-side — see 0x5F). |
...
| 0x35 | SYNCHRONIZE CACHE (10) | 🟩 Yes | O | Real fence — `cache.synchronize_bytes` awaits the cache's flush of every dirty page in the requested LBA range through to storage-backend ack via `VolumeWriter::write_page`. A page whose storage upload failed fails the fence with HARDWARE ERROR / INTERNAL TARGET FAILURE (0x44/0x00 — a backend fault, not medium corruption; issue #109) after re-arming the upload, so a retried fence drives a fresh attempt. Reservation-gated as a write-side opcode. |
...
Every data-path opcode above maps a failing `PageCache` call through one
shared fault classifier (`core_block::FaultClass` — the NVMe dispatcher
keys off the same one, so the two transports cannot disagree on the
same fault; issue #109). The split mirrors what the tape application
reports for the same physical faults:

- **Integrity faults** — the stored payload or its metadata is corrupt:
  a refetched chunk fails its BLAKE3 content-hash check, or AES-GCM
  authentication fails on decrypt. These are medium faults: MEDIUM
  ERROR (0x03) + UNRECOVERED READ ERROR (0x11/0x00) on the read side,
  WRITE ERROR (0x0C/0x00) on the write side. Hosts treat them as
  per-block read/write failures (retire the affected data), not a
  broken device.
- **Infrastructure faults** — the target failed, not the medium: a
  local pread/pwrite EIO on `pages.idx` / the cache file / a pool
  chunk, a storage-backend op failure (including a fence that found
  failed uploads), daemon shutdown mid-command, or a keystore fault
  (missing/short DEK). These return HARDWARE ERROR (0x04) + INTERNAL
  TARGET FAILURE (0x44/0x00) — the same sense VTL gives a pread EIO —
  so hosts fail over / retire the device rather than the medium.
- **Backpressure** stays NOT READY + OPERATION IN PROGRESS (0x04/0x07,
  transient — the host retries), and request-shape errors that should
  have been caught upstream stay ILLEGAL REQUEST + INVALID FIELD IN
  CDB (defensive).
