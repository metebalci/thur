...
| 0x35 | SYNCHRONIZE CACHE (10) | 🟩 Yes | O | Real fence — `cache.synchronize_bytes` awaits the cache's flush of every dirty page in the requested LBA range through to storage-backend ack via `VolumeWriter::write_page`. A page whose storage upload failed fails the fence with MEDIUM ERROR / WRITE ERROR (0x0C/0x00) after re-arming the upload, so a retried fence drives a fresh attempt. Reservation-gated as a write-side opcode. |
