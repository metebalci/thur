...
|-------:|---------|--------|:----:|-------|
| 0x00 | Flush | 🟩 Yes | M | `PageCache::synchronize_bytes(0, size_bytes)` — a real fence, awaits storage-backend ack of every dirty page. A page whose storage upload failed fails the Flush with `Internal Error` after re-arming the upload, so a retried Flush drives a fresh attempt. NSID 0xFFFFFFFF (broadcast) accepted on the live namespace; one namespace per `PageCache` so no per-controller global flush is needed. |
| 0x01 | Write | 🟩 Yes | M | `PageCache::write_bytes(slba * lba_bytes, data_out)`. Sub-page writes via cache RMW. WORM volumes refuse with `Namespace Is Write Protected` (SCT=Generic, SC=0x20, DNR=1) — the NVMe analog of the SBC WRITE PROTECTED gate (issue #79). |
...
| 0x08 | Write Zeroes | 🟩 Yes | O | With the DEAC bit (CDW12 bit 25) set, deallocates via `PageCache::unmap_bytes` (fast index-only path; the range still reads back zero) — Linux sets DEAC for `fallocate(ZERO_RANGE)` / `blkdiscard -z`. Without DEAC, zero-fills via `PageCache::write_bytes` (sub-page zeroing via cache RMW). WORM refuses with `Namespace Is Write Protected` (Generic 0x20). |
| 0x09 | Dataset Management | 🟩 Partial | O | AD (Deallocate, CDW11 bit 2) only — each 16-byte range descriptor is walked in 256 MiB byte windows (`PageCache::unmap_bytes(off, len)` per window) with a runtime yield between windows, so one DSM command can't monopolize a tokio worker even if the host ignored the advertised DMRSL (CNS 0x06). A deallocate against a WORM volume refuses with `Namespace Is Write Protected` (Generic 0x20). Without AD, IDR / IDW hint passes return success as a no-op — even on WORM, since they don't mutate. |
...
| 0x0E | Reservation Report | 🟩 Yes | O | Reservation Status Data Structure from a snapshot of the shared state, byte-exact to NVMe Base 1.4 §6.13. EDS (CDW11 bit 0) selects the extended form: a 64-byte header (entries start at byte 64) with 64-byte controller entries carrying RKEY at entry bytes 8..16 and the full 128-bit HOSTID at 16..32. The short form uses a 24-byte header (entries at byte 24) with 24-byte entries carrying the low-64 HOSTID at 8..16 and RKEY at 16..24. In both forms CNTLID is at 0..2, RCSTS at 2, bytes 3..8 reserved. PTPLS (byte 9) reflects the namespace's current Persist Through Power Loss state. One registered-controller entry per HOSTID; CNTLID is the registrant's representative live controller (its lowest CNTLID), or `0` if the host has a persisted registration but no live controller. The fencing identity remains the HOSTID, not the CNTLID. |
...
| 0x01 | Identify Controller | 🟩 Yes | VID=0 / SSVID=0 (fabrics-only), CNTLID = the per-controller ID assigned at Connect (distinct per association), CMIC (byte 76) bit 1 set (subsystem may contain two or more controllers; bits 0/3 clear — single port, no ANA), OAES (bytes 92..96) bit 8 set (Namespace Attribute Changed async events supported, so the host enables them via FID 0x0B), VER=`0x00010400` (NVMe 1.4.0), NN from live registry, KAS=120 (12 s), MDTS=8 (1 MiB max transfer at the 4 KiB MPSMIN page), ONCS (bytes 520..522) bits 5 (Reservations) + 2 (Dataset Management) set — the DSM bit is what makes Linux enable discard for the namespace and read the CNS 0x06 DMRSL cap, bringing NVMe thin-provisioning reclaim to parity with the iSCSI/SBC UNMAP surface, SUBNQN=`nqn.2025-10.com.metebalci:thurvsa`, SN = a persistent 20-ASCII-char serial (`THURVSA` + 13 random hex) minted once into `<data_dir>/nvme-serial` and stable across restarts and volume create/destroy, so host `/dev/disk/by-id/nvme-*` symlinks and multipath WWIDs don't churn, SGLS bit 0 set, IOCCSZ=1028 / IORCSZ=1. |
...
| 0x06 | I/O Command Set specific Identify Controller | 🟩 Yes | DMRSL (DSM Range Size Limit, bytes 4..8) = 65 536 logical blocks (256 MiB at the 4 KiB LBA); every other field zero ("no specific NVM limit"). Linux honors DMRSL via `nvme_config_discard` to cap `max_discard_sectors`, so a host `mkfs`/`fstrim` full-device discard is split into bounded DSM Deallocate commands rather than one unbounded range that would run past the host I/O timeout and reset the controller. Linux nvme-tcp issues this against a 1.4-versioned controller during bring-up; refusing it kills the namespace attach. |
...
| 0x06 | Authentication Receive | 🟩 Yes | CC | DH-HMAC-CHAP controller messages (Challenge / Success1 / Failure1) returned as a C2HData PDU + CapsuleResp. The CapsuleResp SQID echoes the queue's QID. The Allocation Length (CDW11) is honored: a message that would exceed it fails the command with `Invalid Field in Command` rather than over-sending (no conformant host trips this — the Challenge is <= ~1.1 KiB). |
