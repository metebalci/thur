...
| 0x09 | Dataset Management | 🟩 Partial | O | AD (Deallocate, CDW11 bit 2) only — each 16-byte range descriptor calls `PageCache::unmap_bytes(slba * lba_bytes, nlb * lba_bytes)`. A deallocate against a WORM volume refuses with `Namespace Is Write Protected` (Generic 0x20). Without AD, IDR / IDW hint passes return success as a no-op — even on WORM, since they don't mutate. |
