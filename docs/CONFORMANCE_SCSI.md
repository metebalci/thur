...
| True async copy-status polling | n/a | Deliberate, not a conformance gap: XCOPY and ODX both complete synchronously because our "copy" is a metadata-only page-hash rebind (microseconds, zero data movement) — there is nothing to report progress on. RRTI returns Done on the first poll with ESTIMATED STATUS UPDATE DELAY = `0` ("poll immediately, already done"), which is spec-valid. Arrays that move real bytes need async to surface progress on multi-minute copies; our dedup chunk-pool model makes every offload near-instant, so the machinery would only ever report "done" a few microseconds later. Tracked here for completeness. |
