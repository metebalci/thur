...
| 0x91 | SPACE(16) | 🟩 Yes | O | 64-bit count. Same residual + direction-aware EOD-vs-BOP termination as SPACE(6): forward shortfall → BlankCheck 0x00/0x05, backward shortfall → NoSense 0x00/0x04 (issue #73). |
...
|-----------|:----:|-------|
| VAAI Hardware Accelerated Copy (XCOPY LID1) | `0x83` sa `0x00` | Cross-LUN supported; cross-LUN fast path requires matching backend + `DedupScope` namespace. |
...
| Hyper-V ODX WRITE USING TOKEN | `0x83` sa `0x11` | Cross-LUN via page-hash rebind; mismatched pool refuses with INVALID FIELD IN PARAMETER LIST. |
...
|-----------|:----:|--------------|
| VAAI XCOPY LID4 | `0x83` sa `0x01` | The richer LID4 parameter list adds nothing ESXi / Windows actually issue. LID1 is the production path. |
...
| ODX CANCEL ROD TOKEN | `0x83` sa `0x12` | Hosts cancel rarely; our TTL sweeper drops abandoned tokens within 5 min. |
...
| ODX persisted tokens across daemon restart | n/a | Token table is in-memory. Daemon restart cancels every outstanding ROD; matches SPC-4's "implementation may invalidate tokens" allowance. |
...
| True async copy-status polling | n/a | XCOPY and ODX both run sync-inline; RRTI returns Done on the first poll. Hosts that gate ESTIMATED STATUS UPDATE DELAY on a nonzero value get `0` (poll immediately). |
| RECEIVE COPY RESULTS sa `0x01` RECEIVE DATA / sa `0x04` FAILED SEGMENT DETAILS / sa `0x05` OPERATIONS COUNT | `0x84` | Not implemented; initiators that need them fall back to host-side accounting. |
