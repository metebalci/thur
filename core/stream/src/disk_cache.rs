...
pub struct DiskCacheManager {
    ...
    ghost_list: Option<std::sync::Arc<shared_pool::GhostList>>,
}
...
impl DiskCacheManager {
    ...
    /// `evict_chunk` will insert the evicted chunk's hash so the
    /// read-path miss site can bucket re-fetch ages.
    pub fn set_ghost_list(&mut self, gl: std::sync::Arc<shared_pool::GhostList>) {
    ...
    fn evict_chunk(
        ...
        if let Some(gl) = self.ghost_list.as_ref() {
            if let Some(hash_bytes) = hex_to_blake3(&candidate.hash) {
