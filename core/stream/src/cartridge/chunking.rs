...
mod tests {
    ...
    /// Regression for the chunks.idx single-writer corruption (issues
    /// #154 / #28): a view-only handle (the upload worker / eviction)
    /// opened on an all-sealed cartridge must NOT append a fresh staging
    /// chunk to chunks.idx. Pre-fix, that unguarded `append` raced the
    /// drive-side primary's own append at the same `next_id`; the loser's
    /// empty-staging record (hash = None) clobbered the primary's
    /// just-sealed chunk, so a later READ of a block in that chunk
    /// returned the wrong bytes at the right length — the source of the
    /// nightly monte-carlo `read_verify` mismatch.
    #[test]
    fn view_only_open_of_all_sealed_does_not_append_to_chunk_index() {
        use super::super::{CartridgeOpenMode, CartridgeOpenOptions};

        let tmp = TempDir::new().unwrap();
        let tapes = tmp.path().join("tapes");

        // create + write + drop → cartridge is all-sealed, with no trailing
        // staging chunk. Mirrors a cartridge sitting unloaded after its last
        // write was flushed — the state the upload worker opens for.
        let root = {
            let mut cart = Cartridge::create_with_chunking(
                &tapes,
                "VIEWIDX",
            ...
            .expect("create");
            cart.write_data(Bytes::from(vec![0xCDu8; 96 * 1024]))
                .expect("write_data");
            cart.root.clone()
            // drop seals the trailing chunk → all sealed.
        };

        let chunks_idx = root.join("chunks.idx");
        let len_before = fs::metadata(&chunks_idx).expect("chunks.idx metadata").len();

        // Open the all-sealed cartridge as a view-only handle (the upload
        // worker's secondary open). It must not grow chunks.idx or create a
        // staging file for the would-be next chunk.
        let next_staging = {
            let view = Cartridge::open_with(
                &tapes,
                "VIEWIDX",
            ...
            .expect("open view-only");
            // The view handle still presents a usable in-memory active chunk
            // (a fresh staging rec that was never written to disk), so reads
            // keep working.
            assert!(view.cur_chunk.hash.is_none());
            assert_eq!(view.cur_chunk.size, 0);
            staging_path(&view.root, view.cur_chunk_id)
        };

        let len_after = fs::metadata(&chunks_idx).expect("chunks.idx metadata").len();
        assert_eq!(
            len_before, len_after,
            "view-only open must not append a staging record to chunks.idx"
        );
        assert!(
            !next_staging.exists(),
            "view-only open must not create a staging file for a new chunk"
        );
    }
}
