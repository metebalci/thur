...
// VSA L2 streaming-write throughput benchmark — same shape as
// perf_volume_write (L1) but the backend is operator-selectable.
// Plumbs PageCache → VolumeWriter → ChunkPool → <configured backend>;
...
// shell-side fixture for L3/L4 so all four rows compare at the same
// workload.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let total_mb: usize = args
        ...
        .unwrap_or(1024);
    let block_kib: usize = args.get(4).map(|s| s.parse().unwrap_or(256)).unwrap_or(256);
    let fixture = args.get(5).map(String::as_str).unwrap_or("random");
    ...
    // flush_all drives every dirty page through the writer → ChunkPool
    // → <backend>. For non-local backends this is where the network
    // cost lands; the write phase only paid the in-RAM cache cost.
    ...
    // Local pool index — for non-local backends this directory exists
    // but only carries pool sidecar state, not the actual chunks; the
    // CHUNKS line is most useful when the configured backend is Local
    // and the harness wants to cross-check L1 vs L2.
