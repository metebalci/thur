...
//! Emits `THURVTL_VERSION=<crate-ver> (<sha>[-dirty])` so clap's
//! `#[command(version = ...)]` in `main.rs` includes the build's git
//! SHA. Same logic lives in `vtl/cli/build.rs`; duplicated
//! deliberately because a shared build-helper crate isn't worth the
//! workspace plumbing for ~30 lines.
//!
...
fn main() {
    ...
    // Layout C puts this crate at `<workspace>/vtl/daemon/`, so
