...
use core_mediachanger::{
    CartridgeFacts, PlannedMoveReport, SkippedCartridge, TieringPlanReport, plan_moves,
    read_cartridge_held,
};
...
pub async fn run(emitter: JobEmitter, _body: serde_json::Value, state: Arc<DaemonState>) {
    ...
    if policies.is_empty() {
        emitter
            .info("no tiering policies configured — nothing to plan")
            .await;
        emit_report(&emitter, TieringPlanReport::default()).await;
        return;
    }
    ...
    let (disk, mut skipped) =
        match tokio::task::spawn_blocking(move || scan_disk_facts(&data_dir)).await {
            Ok(v) => v,
            Err(e) => {
                emitter
                    .emit(JobEvent::done_with_error(2, format!("scan panicked: {e}")))
                    .await;
                return;
            }
        };
