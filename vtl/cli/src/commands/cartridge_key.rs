...
pub async fn cmd_key_migrate(
    ...
    // the type in a `type:` tag (the shipped example names the local
    // entry `local-example`), so keying on the literal "local" left the
    // plaintext DEK sidecar on disk whenever the operator named it
    // anything else — issue #218.
