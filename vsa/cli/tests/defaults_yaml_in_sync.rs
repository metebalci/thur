...
//! Guard rail: keep the checked-in `dist/thurvsa.defaults.yaml` in
...
const DEFAULTS_PATH: &str = "dist/thurvsa.defaults.yaml";

/// Repo root = workspace root. Layout C puts this crate at
/// `<workspace>/vsa/cli/`, so the workspace root is two parents
...
fn repo_root() -> PathBuf {
    ...
    PathBuf::from(manifest_dir)
        .parent()
        .and_then(|p| p.parent())
        .expect("thurvsa must live two levels under the repo root")
        .to_path_buf()
}
    ...
    if on_disk != generated {
        panic!(
            "{} is out of sync with the in-code defaults reference.\n\
             Regenerate it with:\n\
             \n    cargo run -p thurvsa -- config defaults > {}\n",
